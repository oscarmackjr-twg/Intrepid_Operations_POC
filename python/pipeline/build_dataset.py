...
def _clean_exhibit_a(excel_path: str, platform: str) -> pd.DataFrame:
    """
    Load an Exhibit A Excel file with robust header detection and cleanup.

    Handles:
    - Header rows with blank / NaN cells
    - Trailing empty columns
    - Extra rows above the true header
    """
    ...
    for h in raw_headers:
        if h is None or (isinstance(h, float) and pd.isna(h)):
            seen_empty += 1
            headers.append(f"unnamed_{seen_empty}")
            continue

        hs = str(h).strip()
        if hs == "" or hs.lower() == "nan":
            seen_empty += 1
            headers.append(f"unnamed_{seen_empty}")
        else:
            headers.append(hs)

    # Build dataframe below header
    ...
    # Normalize seller loan number to a clean string and drop blanks
    df["SELLER Loan #"] = df["SELLER Loan #"].astype("string").str.strip()
    df = df[df["SELLER Loan #"].notna() & (df["SELLER Loan #"] != "")].copy()
...
def _normalize_colname(df: pd.DataFrame, preferred: str, candidates: Tuple[str, ...]) -> str:
    ...
    Robust to Excel headers that contain blanks (NaN -> float) by stringifying safely.
    """
    cols_lower = {}
    for c in df.columns:
        if c is None:
            continue
        # Pandas will store blank header cells as NaN (float)
        if isinstance(c, float) and pd.isna(c):
            continue
        cs = str(c).strip()
        if cs == "" or cs.lower() == "nan":
            continue
        cols_lower[cs.lower()] = cs
...
def build_dataset(
    ...
    # 🔑 Canonicalize names for rules
    merged = _snake_case_output_columns(merged)

    # 🔑 Add loan_type HERE (after snake_case so it’s deterministic)
