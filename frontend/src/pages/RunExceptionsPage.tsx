  ...
  useEffect(() => {
    ...
    const fetchExceptions = async () => {
      try {
        ...
        console.log("RunExceptionsPage result:", result);
          ...
          >
            <thead>
              <tr>
                ...
                <th style={{ borderBottom: "1px solid #ccc" }}>Balance Impact</th>
                <th style={{ borderBottom: "1px solid #ccc" }}>Created At</th>
              </tr>
            </thead>
            <tbody>
              {rows.map((ex) => (
                <tr key={ex.exception_id}>
                  <td style={{ borderBottom: "1px solid #eee" }}>{ex.seller_loan_no}</td>
                  <td style={{ borderBottom: "1px solid #eee" }}>{ex.exception_type}</td>
                  <td style={{ borderBottom: "1px solid #eee" }}>{ex.rule_id}</td>
                  <td style={{ borderBottom: "1px solid #eee" }}>{ex.severity}</td>
                  <td style={{ borderBottom: "1px solid #eee" }}>{ex.balance_impact}</td>
                  <td style={{ borderBottom: "1px solid #eee" }}>{ex.created_at}</td>
                </tr>
              ))}
            </tbody>
          </table>
